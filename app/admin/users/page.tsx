...
import {
  Users, Search, Monitor, Trash2, RefreshCw, ChevronLeft, ChevronRight
} from "lucide-react";
...
export default function UserManagementPage() {
  ...
  const fetchUsers = useCallback(async () => {
    ...
    try {
      ...
      // If the admin themselves got banned mid-session, redirect them
  ...
  const handleStatusChange = async () => {
    ...
    try {
      ...
      if (actionType === "suspend") {
        ...
        body.suspendUntil = suspendUntil;
      }
  ...
  const handleDeviceBan = async (
    ...
    try {
      ...
      toast.error("Device action failed");
    }
  };
  ...
  const handleDeviceRemove = async (userId: string, deviceId: string) => {
    ...
    try {
      const res  = await fetch(
        `/api/admin/users/${userId}/devices?deviceId=${deviceId}`,
        { method: "DELETE" }
      );
      const data = await res.json();
      if (!data.success) { toast.error(data.message); return; }
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div
        ...
        style={{
          background: "linear-gradient(135deg,rgba(248,113,113,0.10),rgba(124,110,243,0.10))",
      ...
      >
        ...
        <p className="text-[13px] mt-1" style={{ color: "var(--text3)" }}>
          ...
          Bans take effect on the user&apos;s very next request — no delay.
        </p>
      </div>
      ...
      >
        {loading ? (
          ...
          <table className="w-full">
            <thead>
              <tr style={{ borderBottom: "1px solid var(--border2)" }}>
                {["User", "Status", "Last Login", "Devices", "Actions"].map(h => (
                  <th
                    key={h}
                    className="px-4 py-3 text-left text-[11px] font-bold tracking-widest uppercase"
            ...
            {/*
              ── KEY FIX ──
              We cannot put a <tbody> inside a <tbody> (invalid HTML).
              The original code returned <tbody key={user._id}> inside
              the outer <tbody>, which caused the React key warning AND
              invalid DOM nesting.

              Correct fix: use React.Fragment with a key on the fragment,
              and keep the outer <tbody> as the only tbody in the table.
            */}
            <tbody>
              {users.map(user => {
                const effectiveStatus = (user.status ?? "active") as
                  "active" | "suspended" | "banned";
                const sc = STATUS_COLORS[effectiveStatus];

                return (
                  // KEY goes on the Fragment — this is the correct pattern
                  // for rendering multiple <tr> elements per iteration
                  // inside a single <tbody>
                  <tr
                    key={user._id}
                    style={{
                      ...
                      <table className="w-full">
                        <tbody>
                          ...
                          <tr
                            ...
                            onClick={() =>
                              setExpanded(
                                expanded === user._id ? null : user._id
                              )
                            }
                          >
                            ...
                            <td
                              ...
                              style={{ color: "var(--text3)", width: "15%" }}
                            >
                              {user.lastLogin
                            ...
                            <td
                              className="px-4 py-3 text-[12px] font-mono"
                              style={{ color: "var(--text3)", width: "12%" }}
                            >
                          ...
                          {expanded === user._id && (
                            <tr>
                              ...
                              >
                                <p
                                  className="text-[11px] font-bold tracking-widest uppercase mb-3 flex items-center gap-2"
                                  style={{ color: "var(--text4)" }}
                                >
      ...
      {actionUser && actionType && (
        ...
        >
          ...
          >
            ...
            <p
              className="text-[13px] font-mono mb-4"
              style={{ color: "var(--text3)" }}
            >
              {actionUser.fullName || actionUser.email} ({actionUser.email})
            </p>

            {actionType === "ban" && (
              <p
                className="text-[12px] font-mono mb-4 px-3 py-2 rounded-xl"
                style={{
              ...
              >
                ⚡ Takes effect immediately. The user is rejected on their next
                request — no delay.
              </p>
            )}
                ...
                {actionType === "suspend" && (
                  <div>
                    <label
                      ...
                      style={{ color: "var(--text4)" }}
                    >
                      Suspend Until (leave blank = indefinite)
                    </label>
                    <input
                      type="datetime-local"
                      value={suspendUntil}
